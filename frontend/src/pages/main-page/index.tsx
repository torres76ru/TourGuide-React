...
const MainPage = () => {
  ...
  const museums = useSelector(
    (state: RootState) =>
      state.attraction.attractionsByTag["museum"]?.attractions ?? []
  );
  const loadingMuseums = useSelector(
    (state: RootState) =>
      state.attraction.attractionsByTag["museum"]?.loading ?? false
  );
  const errorMuseums = useSelector(
    (state: RootState) =>
      state.attraction.attractionsByTag["museum"]?.error ?? null
  );

  const entertainments = useSelector(
    (state: RootState) =>
      state.attraction.attractionsByTag["amenity"]?.attractions ?? []
  );
  const loadingEntertainment = useSelector(
    (state: RootState) =>
      state.attraction.attractionsByTag["amenity"]?.loading ?? false
  );
  const errorEntertainment = useSelector(
    (state: RootState) =>
      state.attraction.attractionsByTag["amenity"]?.error ?? null
  );

  useEffect(() => {
    dispatch(fetchAttractionsRequest({ tag: "museum" }));
    dispatch(fetchAttractionsRequest({ tag: "amenity" }));
  }, [dispatch]);
