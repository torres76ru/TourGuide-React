...
export default function Review({ name, rating = 0, date, text }: ReviewProps) {
    ...
    return (
        <div className={styles.review}>
            ...
            <div className={expanded ? styles.textExpanded : styles.text} style={{transitionDuration: shouldShowButton ? '0.5s' : '0s'}}>{text}</div>
            {shouldShowButton && (<button className={styles.button} onClick={() => setExpanded(!expanded)}> {expanded ? 'Свернуть' : 'Ещё'} </button>)}
        </div>
    );
}